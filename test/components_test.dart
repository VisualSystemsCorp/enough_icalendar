...
END:VCALENDAR''';
      ...
      test(
        'Unfold line spread across 3 lines',
        () {
          const input = '''DESCRIPTION:This is a lo
 ng description 
...
END:VCALENDAR''';
          final output = VComponent.unfold(input);
          final expected = '''BEGIN:VCALENDAR
VERSION:2.0
PRODID:-//hacksw/handcal//NONSGML v1.0//EN
BEGIN:VEVENT
UID:19970610T172345Z-AF23B2@example.com
...
'''
      ...
      test(
        'Some lines folded with LF linebreks in properties',
        () {
          const input = '''BEGIN:VCALENDAR
METHOD:xyz
VERSION:2.0
...
'''
  ...
  group(
    ...
    () {
      test(
        'Calendar simple - unix linebreaks',
        () {
...
''';
      ...
      test(
        'Recurrent event - yearly',
        () {
          const text = '''BEGIN:VEVENT\r
...
''';
      ...
      test(
        'three-day conference example with CRLF line breaks',
        () {
          const text = '''BEGIN:VCALENDAR\r
...
''';
      ...
      test(
        'group-scheduled meeting with VTIMEZONE',
        () {
          const text = '''BEGIN:VCALENDAR
...
''';
          ...
          expect(event.classification, Classification.public);
          expect(
              event.summary, 'Calendaring Interoperability Planning Meeting');
          expect(event.description,
...
''';
          ...
          expect(
              todo.attendees[0].participantStatus, ParticipantStatus.accepted);
...
''';
          ...
          expect(
              (freebusy as VFreeBusy).organizer?.email, 'jsmith@example.com');
          ...
          expect(freebusy.freeBusyProperties[0].freeBusyType,
...
''';
          ...
          expect(
              journal.categories, ['Project Report', 'XYZ', 'Weekly Meeting']);
          expect(
              journal.description, '''Project xyz Review Meeting Minutes\nAgenda
1. Review of project version 1.0 requirements.
...
END:VCALENDAR''';
          ...
          expect(event, isA<VEvent>());
          expect((event as VEvent).summary,
              'Kostenloser Antigen-Schnelltest Bremen');
          ...
          expect(event.organizer!.uri,
              Uri.parse('MAILTO:sofortsupport@ticket.io'));
...
''';
          ...
          expect(event.organizer, isNotNull);
          expect(event.organizer!.uri,
              Uri.parse('MAILTO:mrs.organizer@example.com'));
  ...
  group(
    ...
    () {
      ...
      test(
        'Full day event with UTC timing',
        () {
          final invite = VCalendar.createEvent(
            organizerEmail: 'a@example.com',
            attendeeEmails: ['a@example.com', 'b@example.com', 'c@example.com'],
